...
6. **Agentic Reasoning Layer** [NEW] — Utilizes LlamaIndex `SubQuestionQueryEngine` to handle multi-hop queries. It decomposes complex questions into logical sub-tasks, retrieves across multiple sources, and synthesizes a final cohesive answer.

7. **Multimodal Ingestion Layer** [NEW] — Extends the pipeline to handle images (`.png`, `.jpg`, `.jpeg`) using Tesseract OCR (via `ImageReader`) to extract textual context from visual data.

8. **Observability Layer** [NEW] — Integrated Arize Phoenix for deep tracing of retrieval spans, latency breakdowns, and token cost attribution.

9. **Isolation & Scoping Layer** [NEW] — Implements strict document isolation via session-aligned metadata filters. Ensures that in multi-document environments, queries are strictly scoped to the active file, preventing cross-document information "spillage".

10. **Dynamic Intent Dispatcher** [NEW] — Analyzes user queries for depth intent. Standard queries use metadata-enriched summaries, while requests for "detailed explanations" trigger a deep RAG path with increased `top_k` and specialized analyst prompts.

11. **Streaming Post-Processor** [NEW] — A frontend-side cleanup layer that corrects Markdown header formatting and normalizes citation clusters (e.g., `[Source , Source]`) in real-time as the LLM streams.
