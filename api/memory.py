...
class MemoryManager:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning("firebase_initialization_failed", error=str(e), hint="System will use local JSON for persistent memory")
    ...
    def get_history(self, session_id: str):
        ...
        if not self.enabled:
            return []
        try:
            docs = self.db.collection("sessions").document(session_id).collection("messages").order_by("timestamp").stream()
