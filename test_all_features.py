...
if __name__ == "__main__":
    ...
    test_endpoint("Conversational Query", {"question": "Hello! How are you today?", "session_id": "test_routing"})

    # 2. RAG Query (should use HyDE and Hybrid Search)
    test_endpoint(
        "Technical RAG Query",
        {"question": "What is the transformer architecture and how is it used in RAG?", "session_id": "test_rag"},
    )

    # 3. Guardrail Trigger (should be rejected)
    test_endpoint(
        "Malicious/Guardrail Query",
        {
    ...
    test_streaming(
        {"question": "Explain the role of embeddings in a RAG production system.", "session_id": "test_streaming"}
    )
